...
export default class Uploader extends React.Component {
    ...
    saveToServer = () => {
        // this.props.toast("Saving album data", 'dark')
		const albumId = this.props.albumId
		const sessionId = this.props.sessionId

		fetch( c.apiUrl + '/edit-album/settings', {
			method: 'POST',
			headers: {
		...
		.then( json => {
			if( json.status ){
                this.props.toast("Album Data Saved Successfully", 'dark')
		...
		.catch( err => {
			this.props.toast("Unable to send data to server", 'error')
            this.props.savingStatusToggler( false )
		})
	}
